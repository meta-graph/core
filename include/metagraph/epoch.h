...
static inline void mg_attachment_epoch_init(mg_attachment_epoch_t *e) {
    atomic_store(&e->epoch, 1);
}
static inline uint64_t
mg_attachment_epoch_load(const mg_attachment_epoch_t *e) {
    return atomic_load(&e->epoch);
}
static inline void mg_attachment_epoch_flip(mg_attachment_epoch_t *e) {
    atomic_fetch_add(&e->epoch, 1);
}
