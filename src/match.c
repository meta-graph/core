...
/**
 * Initialize an mg_match_set_t structure and optionally allocate its internal storage.
 *
 * The function zeroes the provided structure and, if capacity > 0, allocates an array
 * of mg_match_t of the requested size and sets the structure's capacity accordingly.
 *
 * @param set Pointer to the mg_match_set_t to initialize (must not be NULL).
 * @param capacity Initial number of elements to allocate; if zero no allocation is performed.
 * @returns `true` if the structure was initialized and any requested allocation succeeded, `false` otherwise.
 */
 ...
 *
 * Allocates or grows the internal storage so that the set can contain at least min_capacity elements and updates set->capacity on success; no allocation is performed if the current capacity already meets or exceeds min_capacity.
 *
 * @param set Pointer to the match set to modify; if NULL the function does nothing and returns `false`.
 * @param min_capacity Minimum required number of elements the set must be able to hold.
 * @returns `true` if the set has at least min_capacity capacity after the call, `false` on allocation failure or when `set` is NULL.
 */
