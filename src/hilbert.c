...
/**
 * Initialize a Hilbert register handle with storage for a given number of nodes.
 *
 ...
 * @param count Number of node entries to allocate (zero is allowed and allocates a minimal buffer).
 * @returns `METAGRAPH_OK()` on success; `METAGRAPH_ERR(METAGRAPH_ERROR_NULL_POINTER, ...)` if `hilbert` is NULL;
 *          `METAGRAPH_ERR(METAGRAPH_ERROR_OUT_OF_MEMORY, ...)` if allocation fails.
 */
 ...
 *          Returns `METAGRAPH_ERR(METAGRAPH_ERROR_NULL_POINTER, ...)` if `hilbert` is NULL.
 *          Returns `METAGRAPH_ERR(METAGRAPH_ERROR_OUT_OF_MEMORY, ...)` if memory allocation fails.
 */
