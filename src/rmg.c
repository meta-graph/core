 ...
 * @param[out] kind Receives the attachment kind; set to `MG_ATT_NONE` when no
 * attachment is present.
 * @returns `true` on successful query (including missing/out-of-range
 * attachments), `false` if input pointers are NULL.
 */
 ...
 * @param attachment Output pointer set to the hydrated attachment pointer, or
 * NULL if no attachment exists or the index is out of range.
 * @returns `true` on successful query (including when no attachment is
 * present), `false` if input pointers are NULL.
 */
